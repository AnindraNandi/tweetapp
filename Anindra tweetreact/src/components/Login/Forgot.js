...
class Forgot extends Component {
  ...
  getUserDetails = (event) => {
    axios
      .get(
        `https://cors-everywhere.herokuapp.com/http://tweetapp-env.eba-au9yqf7p.ap-northeast-1.elasticbeanstalk.com/api/v1.0/tweets/${this.email.current.value}/forgot`
      )
      .then((res) => {
        this.setState({
  ...
  updatePassword = (event) => {
    ...
    if (this.password1.current.value === this.password2.current.value) {
      ...
      axios
        .put("https://cors-everywhere.herokuapp.com/http://tweetapp-env.eba-au9yqf7p.ap-northeast-1.elasticbeanstalk.com/api/v1.0/tweets/updateUser", body, config)
        .then((res) => {
